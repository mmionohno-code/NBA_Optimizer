...
BASE_ARCHETYPE_MINIMUMS = {
    'Perimeter Playmaker':   2,   # need ball handlers
    '3-and-D Wing':          2,   # perimeter depth
...
SCENARIOS = {
    ...
    'D': {
        ...
        'objective': 'composite',
        'variant':   'rebuild',
        'note':       'Tanking / rebuild team. Hard cap + must include ≥5 players age ≤ 24.'
    },
    'E': {
        'name':      'E — Win-Now ($165M)',
        'cap':        165_294_000,
        'objective': 'composite',
        'variant':   'win_now',
        'note':       'Championship window. Luxury cap + must include ≥5 veterans age ≥ 28.'
    },
    'F': {
    ...
    'H': {
        ...
        'objective': 'composite',
        'variant':   'three_point',
        'note':       'Modern spacing team. Hard cap + ≥6 players shoot above league-avg FG3%.'
    },
    'I': {
        ...
        'objective': 'composite',
        'variant':   'small_ball',
        'note':       'No traditional bigs. Hard cap + max 3 total big men on roster.'
    },
    'J': {
...
for key, cfg in SCENARIOS.items():
    ...
    if variant == 'small_ball':
        archetype_mins.pop('Rim Protector',     None)
        archetype_mins.pop('Elite Two-Way Big',  None)
        archetype_mins.pop('Stretch Big',        None)
    ...
    max_bigs = 3 if variant == 'small_ball' else 7
    ...
    if variant == 'rebuild':
        # Must have at least 5 players age 24 or younger
        young_indices = df_2324[df_2324['AGE'] <= 24].index.tolist()
        prob += (lpSum(x[i] for i in young_indices) >= 5, "MinYoungPlayers")
        print(f"  Rebuild constraint: ≥5 players age ≤ 24  ({len(young_indices)} eligible)")

    elif variant == 'win_now':
        # Must have at least 5 veterans age 28 or older
        vet_indices = df_2324[df_2324['AGE'] >= 28].index.tolist()
        prob += (lpSum(x[i] for i in vet_indices) >= 5, "MinVeterans")
        print(f"  Win-now constraint: ≥5 players age ≥ 28  ({len(vet_indices)} eligible)")

    elif variant == 'three_point':
        # Must have at least 6 players shooting above league-avg FG3%
        shooter_indices = df_2324[df_2324['FG3_PCT'] >= league_avg_fg3].index.tolist()
        prob += (lpSum(x[i] for i in shooter_indices) >= 6, "MinThreePointShooters")
        print(f"  Three-point constraint: ≥6 players FG3% ≥ {league_avg_fg3:.3f}  ({len(shooter_indices)} eligible)")
