...
Full methodology writeups live in [`docs/pipeline/`](docs/pipeline/).
...
│   ├── charts/        Methodology figures (PCA, clusters, archetypes)
│   └── tableau/       Tableau-ready CSVs + setup guide
│
...
- **Interactive dashboard:** [`deliverables/dashboards/NBA_Enhanced_Dashboard.html`](deliverables/dashboards/NBA_Enhanced_Dashboard.html) — open in a browser
- **Excel workbook:** [`deliverables/excel/NBA_Optimizer_Dashboard.xlsx`](deliverables/excel/NBA_Optimizer_Dashboard.xlsx)
- **Slide deck:** [`deliverables/presentation/NBA_Optimizer_Presentation.pptx`](deliverables/presentation/NBA_Optimizer_Presentation.pptx)
- **Methodology figures:** [`deliverables/charts/`](deliverables/charts/)
- **Pipeline writeups:** [`docs/pipeline/`](docs/pipeline/)
...
```bash
python 01_feature_engineering.py    # ingest, shrink, PCA, residualize
python 02_clustering.py             # K-Means archetypes
python 03_fix_labels.py             # readable labels
python 04_optimizer_expanded.py     # baseline MILP
python 05_compute_synergy.py        # pairwise synergy
python 06_validate_synergy.py
python 07_optimizer_synergy.py      # full optimizer
python 08_build_dashboard.py
python 09_verify_pipeline.py
```

The SQL database can be regenerated with `python build_database.py` and explored via [DB Browser for SQLite](https://sqlitebrowser.org) by opening [`sql/nba_optimizer.sqbpro`](sql/nba_optimizer.sqbpro). Example queries are in [`sql/sql_queries.sql`](sql/sql_queries.sql).
