...
CLUSTER_STATS = [
    ...
    'INFLUENCE_SCORE',       # USG% x TS% — productive offensive load
    'ON_OFF_DIFF',           # team net rating differential with/without player
    # DREB_PCT_ADJUSTED removed: position-group adjustment showed it fails Spearman
    # validation (rho=0.025, p=0.365) — positional inflation was the confound
]

# Drop rows with missing values in any cluster stat
cols_needed = CLUSTER_STATS + ['PLAYER_NAME', 'PLAYER_ID', 'TEAM_ABBREVIATION',
                               'SEASON', 'COMPOSITE_SCORE_NORM',
                               'SALARY', 'VORPD', 'IS_SHOOTER', 'SEASON_WEIGHT']
cols_needed = [c for c in cols_needed if c in df.columns]
df_cluster = df[cols_needed].dropna()
...
for cluster_id in range(K):
    ...
    elif 'BLK' in defining[:2] and 'DEF_RATING_ADJUSTED' in defining:
        label = 'Rim Protector'
...
plt.tight_layout()
plt.savefig('charts/cluster_visualization.png')
plt.close()
...
stats_for_radar = ['OFF_RATING_ADJUSTED', 'DEF_RATING_ADJUSTED',
                   'AST_PCT_ADJUSTED', 'BLK', 'STL',
                   'TS_PCT', 'FG3_PCT', 'INFLUENCE_SCORE', 'ON_OFF_DIFF']
...
plt.tight_layout()
plt.savefig('charts/archetype_radar_charts.png', bbox_inches='tight')
plt.close()
